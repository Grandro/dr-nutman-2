...
class_name FWLootRewardsLoot

signal completed()

@export var _e_include_coins: bool = false
@export var _e_item_entry_scene: PackedScene = preload("uid://iqhxv0203ncy")

var _a_Coin_Image: Texture2D = preload("uid://cqok6grjt0dwl")
...
func close() -> void:
	for child: FWItemEntryLoot in get_children():
		child.queue_free()
...
func _instantiate_items(p_loot: Dictionary[StringName, int]) -> void:
	...
	for item_key: StringName in p_loot:
		...
		var instance: FWItemEntryLoot = _e_item_entry_scene.instantiate()
		if item_key == &"$Coins":
			...
			var item_args: FWItemData = items_data[item_key]
