extends Node
class_name ABSBattle

signal battle_started()
signal battle_ended()

var _a_Popup_Scene: PackedScene = preload("res://Global_Scenes/Battle_System/Popup.tscn")

const _a_MAIN_PATH: String = "Navigation/Tilemaps/Same_As_1"

@onready var _a_Party_Member_HUD: ABSPartyMemberHUD = get_node("Party_Member_HUD")
...
func add_instance_main(p_instance) -> void:
	_a_main.add_child(p_instance)

func instantiate_popup(p_to, p_type: StringName, p_text: String) -> void:
...
func deal_damage(p_from, p_to, p_velocity, p_strength) -> void:
...
func is_point_in_range(p_center, p_pos, p_radius) -> bool:
	return p_center.distance_to(p_pos) <= p_radius

func is_any_enemy_in_range(p_center, p_radius) -> bool:
...
func is_any_party_member_in_range(p_center, p_radius) -> bool:
...
func _on_Scene_Manager_scene_changed(p_curr_scene) -> void:
	return
...
func _on_Encounter_area_entered(p_encounter) -> void:
...
func _on_Encounter_area_exited() -> void:
...
func _on_Enemy_died(p_instance) -> void:
	print(p_instance.get_name(), " died.")

func _on_Party_Member_died(p_instance) -> void:
	print(p_instance.get_name(), " died.")
