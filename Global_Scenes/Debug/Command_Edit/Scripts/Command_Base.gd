extends Control
class_name DebugCommandEditCommandBase

signal ok_pressed(p_data: Dictionary)

const _a_TITLE_LOC_ID: String = "DEBUG_CUTSCENES_COMMANDS_%s"
...
func open(_p_instance: DebugCommandEditorEntryBase, p_data: Dictionary, p_res_data: Dictionary) -> void:
	if p_data.is_empty():
