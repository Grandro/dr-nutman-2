...
class_name FWItemSelectTabEntry
...
var _a_Group_Entry_Scene: PackedScene = preload("uid://mfnt2ux5uio8")

@onready var _a_Icons: MarginContainer = get_node("VBox/Icons")
@onready var _a_Icons_Toggler: FWKeyEntryToggler = get_node("VBox/Icons/Toggler")
...
func _ready() -> void:
	...
	for i: int in _e_groups.size():
		...
		if _e_has_icons:
			...
			var instance: FWEntryTogglerKeyEntry = _a_Icons_Toggler.instantiate_entry_("", texture, group)
			_a_Icons_Toggler.add_entry(instance)
	
	_a_Icons.set_visible(_e_has_icons)
	_a_HSep.set_visible(_e_has_icons)

func add_item(p_instance: FWItemEntryInventory, p_group: StringName) -> void:
	var group: HFlowContainer = _a_groups[p_group]
	group.add_child(p_instance)

func clear_items() -> void:
	for group: HFlowContainer in _a_groups.values():
		for child: FWItemEntryInventory in group.get_children():
			child.queue_free()

func move_item_curr(p_instance: FWItemEntryInventory, p_idx: int) -> void:
...
func get_first_item_curr() -> FWItemEntryInventory:
	var first: FWItemEntryInventory = null
	for child: FWItemEntryInventory in _a_group_instance.get_children():
...
func _on_Icons_Toggler_toggled(p_instance: FWEntryTogglerKeyEntry) -> void:
