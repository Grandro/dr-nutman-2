...
class_name MiniGameColorSelectionGamePlay
...
@onready var _a_Floor_Area_Left: Area2D = get_node("Node2D/Floor_Areas/Left")
@onready var _a_Floor_Area_Right: Area2D = get_node("Node2D/Floor_Areas/Right")
@onready var _a_Floor_Area_Glass_Bowl: Area2D = get_node("Node2D/Floor_Areas/Glass_Bowl")
@onready var _a_Check_End_Timer: Timer = get_node("Node2D/Check_End_Timer")
@onready var _a_Force_End_Timer: Timer = get_node("Node2D/Force_End_Timer")

var _a_colors: Array[Color] = [Color.RED, Color.GREEN, Color.BLUE] # Array to select random colors
var _a_color: Color # Current randomly selected color
var _a_glass_bowl_instances: Array[MiniGameColorSelectionPaintDrop] = [] # Paint_Drop instances currently in glass bowl
var _a_spurt_count: int = 0 # Count spurt sequences
...
func open_(p_show_explanation: bool) -> void:
	...
	if p_show_explanation:
		...
		var key: StringName = &"Color_Selection_1"
...
func _finish() -> void:
	...
	var final_color_vec: Vector3 = Vector3.ZERO
	for instance: MiniGameColorSelectionPaintDrop in _a_glass_bowl_instances:
		var color: Color = instance.get_color()
		final_color_vec.x += color.r
		final_color_vec.y += color.g
		final_color_vec.z += color.b
	final_color_vec /= _a_glass_bowl_instances.size()
	var final_color: Color = Color(final_color_vec.x, final_color_vec.y, final_color_vec.z)
...
func _select_rndm_color() -> Color:
	randomize()
	
	_a_colors.shuffle()
	var color: Color = _a_colors.pop_back()
...
func _on_Syringe_request_paint_drop(p_pos: Vector2) -> void:
	var instance: MiniGameColorSelectionPaintDrop = _a_Paint_Drop_Scene.instantiate()
...
func _on_Syringe_completed_spurt_sequence() -> void:
	_a_spurt_count += 1
...
func _on_Floor_Area_Side_body_entered(p_instance: MiniGameColorSelectionPaintDrop) -> void:
	p_instance.set_on_floor(true)

func _on_Floor_Area_Side_body_exited(p_instance: MiniGameColorSelectionPaintDrop) -> void:
	p_instance.set_on_floor(false)

func _on_Floor_Area_Glass_Bowl_body_entered(p_instance: MiniGameColorSelectionPaintDrop) -> void:
	p_instance.set_on_floor(true)
	_a_glass_bowl_instances.push_back(p_instance)

func _on_Floor_Area_Glass_Bowl_body_exited(p_instance: MiniGameColorSelectionPaintDrop) -> void:
	p_instance.set_on_floor(false)
	_a_glass_bowl_instances.erase(p_instance)

func _on_Check_End_Timer_timeout() -> void:
	var finished_: bool = true
	for child: MiniGameColorSelectionPaintDrop in _a_Paint_Drops.get_children():
...
func _on_Force_End_Timer_timeout() -> void:
	_a_Check_End_Timer.stop()
	_finish()
