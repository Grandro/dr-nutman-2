extends Node
class_name FWSceneManager

signal scene_changing()
signal scene_changed(p_instance: Node, p_loaded_file_data: bool)

var _a_Scene_Loader_Scene: PackedScene = preload("uid://cax85npdebukv")

@onready var _a_Scene_Loaders: Node = get_node("Scene_Loaders")
@onready var _a_Loading_Screen: FWSceneManagerLoadingScreen = get_node("Loading_Screen")
...
func change_scene_uid(p_uid: String, p_scene_ready_cb: Callable = Callable()) -> void:
	_a_dest = []
	_a_tp = &""
	_a_uid = p_uid
	_a_scene_ready_cb = p_scene_ready_cb
...
func load_scene(p_path: String, p_scene_loaded_cb: Callable, p_show_loading: bool) -> void:
	var instance: FWSceneManagerSceneLoader = _a_Scene_Loader_Scene.instantiate()
...
func _scene_changed() -> void:
	if is_curr_scene_map():
		...
		if _a_curr_scene_instance is FWMapBase3D:
...
func _update_teleport() -> void:
	if !_a_dest.is_empty():
		var data: FWDestinationDataBase = Databases.get_teleport_data(_a_dest)
		_update_camera(data)
		_update_player(data)

func _update_camera(p_data: FWDestinationDataBase) -> void:
	var global_si: Global = Global.get_singleton(self, "Global")
...
func _update_player(p_data: FWDestinationDataBase) -> void:
	var global_si: Global = Global.get_singleton(self, "Global")
	var player: Node = global_si.get_object(&"Player")
...
func get_curr_scene_dim() -> StringName:
	if _a_curr_scene_instance is FWMapBase2D: return &"2D"
	if _a_curr_scene_instance is FWMapBase3D: return &"3D"
...
func is_curr_scene_map() -> bool:
	if _a_curr_scene_instance is FWMapBase2D: return true
	if _a_curr_scene_instance is FWMapBase3D: return true
...
func _on_Scene_tree_exited(p_type: StringName) -> void:
	match p_type:
		&"Dest":
			...
			var data: FWMapData = Databases.get_data_entry(&"Maps", location)
			var uid: String = data.get_uid()
			load_scene(uid, _CB_Scene_Manager_scene_loaded, true)
		&"Tp":
			var data: FWMapData = Databases.get_data_entry(&"Maps", _a_tp)
			var uid: String = data.get_uid()
			load_scene(uid, _CB_Scene_Manager_scene_loaded, true)
		&"Path":
			load_scene(_a_uid, _CB_Scene_Manager_scene_loaded, true)
