extends Node
class_name DebugTeleportMap

@export var _e_free_cameras: Dictionary[StringName, PackedScene] = {} # Match dim to scene

const _a_DIMENSIONS_PATH: String = "res://Global_Scenes/Debug/Teleport_Map/Dimensions/%s.gd"

var _a_Select_Point: Texture2D = preload("res://Global_Resources/Sprites/Debug/Cursors/Select_Point.png")
...
func _input(p_event: InputEvent) -> void:
	if p_event.is_action_pressed(&"Mouse_Left"):
		...
		if global_pos:
			var player: Node = Global.get_player()
...
func _instantiate_dimensions() -> ExtensionBase:
	var dim: StringName = Scene_Manager.get_curr_scene_dim()
	var script: GDScript = load(_a_DIMENSIONS_PATH % dim)
	var instance: ExtensionBase = script.new(self)
