...
@onready var _a_Properties_Heading: Label = get_node("Margin/HBox/Properties/Heading")
@onready var _a_Properties: DebugEntryList = get_node("Margin/HBox/Properties/Entries")
@onready var _a_Methods_Heading: Label = get_node("Margin/HBox/Methods/Heading")
@onready var _a_Methods: DebugEntryList = get_node("Margin/HBox/Methods/Entries")
...
func update_list(p_instance: Node) -> void:
	...
	for args: Dictionary in properties:
		...
		var instance: DebugEntryListEntry = _a_Properties.instantiate_entry(property)
	...
	for args: Dictionary in methods:
		...
		var instance: DebugEntryListEntry = _a_Methods.instantiate_entry(method)
		instance.set_name_clip_text.call_deferred(true)
...
func _on_Properties_entry_select_pressed(p_instance: DebugEntryListEntry) -> void:
	var property: StringName = p_instance.get_name_()
	property_selected.emit(property)
	close()

func _on_Methods_entry_select_pressed(p_instance : DebugEntryListEntry) -> void:
