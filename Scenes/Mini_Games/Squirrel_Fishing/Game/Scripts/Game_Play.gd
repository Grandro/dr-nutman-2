...
const _a_MOVE_SQUIRREL_FRAME_AMOUNTS: Array[int] = [1, 3, 6]
...
@onready var _a_Audio_Fun_Up: AudioStreamPlayer = get_node("Audio/Fun_Up")
@onready var _a_Audio_Whoosh: AudioStreamPlayer = get_node("Audio/Whoosh")
@onready var _a_Audio_Laugh: AudioStreamPlayer = get_node("Audio/Laugh")
@onready var _a_Anims: AnimationPlayer = get_node("Anims")
@onready var _a_Delay: Timer = get_node("Delay")

var _a_curr_round: int = 0
var _a_curr_squirrel: int = 0
...
func _finish_round() -> void:
	...
	var key: StringName = &"Squirrel_Fishing_Round_Finish"
...
func _process_catch_pressed() -> void:
	set_process_unhandled_input(false)
	_stop_squirrel_hands()
	
	var frame: int = _a_Squirrel_Body.get_frame()
	if frame == 0:
...
func _move_squirrel_hands() -> void:
	var dir: int = _get_rndm_move_squirrel_dir()
	var frame_amount: int = _get_rndm_move_squirrel_frame_amount(dir)
	_tween_squirrel_frame(dir, frame_amount)

func _tween_squirrel_frame(p_dir: int, p_frame_amount: int) -> void:
	var curr_frame: int = _a_Squirrel_Body.get_frame()
	var new_frame: int = curr_frame + p_dir * p_frame_amount
	var duration: float = _e_time_per_frame * p_frame_amount
...
func _stop_squirrel_hands() -> void:
	if _a_tween != null:
		_a_tween.kill()
	_a_Delay.stop()

func _set_squirrel_frame(p_frame: int) -> void:
	_a_Squirrel_Body.set_frame(p_frame)
	_a_Squirrel_Hands.set_frame(p_frame)

func _get_rndm_move_squirrel_dir() -> int:
	var frame: int = _a_Squirrel_Body.get_frame()
	var idx: int = -1
	...
	var dir: int = _a_MOVE_SQUIRREL_DIRS[idx]
	
	return dir

func _get_rndm_move_squirrel_frame_amount(p_dir: int) -> int:
	var frame: int = _a_Squirrel_Body.get_frame()
	var diff: int
	match p_dir:
		-1: diff = frame
	...
	var mod: int = 0
	var frame_amounts_size: int = _a_MOVE_SQUIRREL_FRAME_AMOUNTS.size()
	for i: int in range(frame_amounts_size, 0, -1):
		var frame_amount: int = _a_MOVE_SQUIRREL_FRAME_AMOUNTS[i - 1]
		if diff >= frame_amount:
			mod = i
			break
	
	var frame_amount: int = 0
	if mod != 0:
		var idx: int = randi() % mod
	...
	if p_key == &"Squirrel_Fishing_Round_Finish":
		match p_entry_key:
			&"Entry":
				...
				var amount: int = _a_Cracked_Peanut_Amount.get_amount()
				if amount > 0:
					set_process_unhandled_input(true)
					_move_squirrel_hands()
...
func _on_Tween_finished() -> void:
	var delay: float = randf_range(_e_delay_min, _e_delay_max)
