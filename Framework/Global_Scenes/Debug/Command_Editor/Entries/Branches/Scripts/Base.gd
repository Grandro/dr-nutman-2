...
class_name FWDebugCommandEditorBranchBase
...
var _a_Collapse_Image: Texture2D = preload("uid://c7fr1xnoi2b08")
var _a_Expand_Image: Texture2D = preload("uid://caphhrinp5ctx")
...
func get_entry(p_idx: int) -> FWDebugCommandEditorEntryBase:
...
func get_cutscene_data() -> Array[Dictionary]:
	...
	for child: FWDebugCommandEditorEntryBase in _a_Entries.get_children():
		if !child.is_empty():
			var cutscene_data: Array[Dictionary] = child.get_cutscene_data()
			data.append_array(cutscene_data)
