...
var _a_Branch_Scene: PackedScene = load("res://Global_Scenes/Cutscene_System/Branch.tscn")

@onready var _a_Branches: Node = get_node("Branches")
@onready var _a_Threads: Node = get_node("Threads")

var _a_thread_paths: Dictionary # Scene paths to Threads 2D/3D
...
func skip() -> void:
	for child: CutsceneBranch in _a_Branches.get_children():
		child.skip.call_deferred()
	for child: CutsceneThreadBase in _a_Threads.get_children():
		child.skip.call_deferred()
...
func _process_next_command(p_inc: bool) -> void:
	...
	match _a_command:
		...
		_: 
			var instance: CutsceneThreadBase = _instantiate_thread(data[&"Data"], _a_command)
...
func _process_cond_branch(p_data: Dictionary, p_args: Dictionary) -> void:
	...
	if branches.has(branch):
		...
		var instance: CutsceneBranch = _instantiate_branch(branch, branch_data)
...
func _process_teleport(p_data: Dictionary, p_args: Dictionary) -> void:
	...
	match _a_tp_type:
		&"Map":
			var instance: CutsceneThreadBase = _instantiate_thread(p_data, _a_command)
			_a_Threads.add_child(instance)
...
func _process_match(p_data: Dictionary, p_args: Dictionary) -> void:
	...
	var branches: Dictionary = p_args[&"Branches"]
	var branch_data: Array[Dictionary]; branch_data.assign(branches[branch][&"Entries"])
	var branch_instance: CutsceneBranch = _instantiate_branch(branch, branch_data)
...
func _process_loop(p_data: Dictionary, p_args: Dictionary) -> void:
	...
	var instance: CutsceneBranch = _instantiate_branch(0, branch_data)
	_a_Branches.add_child(instance)
...
func _instantiate_branch(p_branch: int, p_data: Array[Dictionary]) -> CutsceneBranch:
	var data_origin: Array[Array] = _a_data_origin.duplicate()
	data_origin.push_back([_a_data_idx, p_branch])
	
	var instance: CutsceneBranch = _a_Branch_Scene.instantiate()
...
func _instantiate_thread(p_args: Dictionary, p_command: StringName, p_loads: bool = false) -> CutsceneThreadBase:
	var path: String = _a_thread_paths[p_command][_a_dim]
	var scene: PackedScene = load(path)
	var instance: CutsceneThreadBase = scene.instantiate()
	...
	match _a_tp_type:
		&"Battle":
			if _a_tp_branches.has(_a_tp_branch):
				...
				var instance: CutsceneBranch = _instantiate_branch(_a_tp_branch, branch_data)
				_a_Branches.add_child(instance)
			else:
				_process_next_command(true)
...
func set_thread_paths(p_thread_paths: Dictionary) -> void:
...
func _load_threads_data(p_data: Array[Dictionary]) -> void:
	for i: int in p_data.size():
		...
		var instance: CutsceneThreadBase = _instantiate_thread(args, command, true)
		if i == p_data.size() - 1:
...
func _on_Branch_command_changed(p_instance: CutsceneBranch) -> void:
...
func _on_Thread_ready(p_instance: CutsceneThreadBase) -> void:
	var command: StringName = p_instance.get_command()
	var idx: int = p_instance.get_index()
	p_instance.set_name("%s_%d" % [command, idx])
