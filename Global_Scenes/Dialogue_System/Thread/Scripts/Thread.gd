extends Node
class_name DialogueSystemThread

signal completed()

const _a_PROCESS_SCENE_PATH: String = "res://Global_Scenes/Dialogue_System/Thread/Process/Process_%s.tscn"
...
func manual_proceed() -> void:
	var child: DialogueSystemThreadProcessBase = _a_Processes.get_child(0)
	child.queue_free()
...
func _process_next() -> void:
	...
	var type: StringName = args[&"Type"]
	var instance: DialogueSystemThreadProcessBase = _instantiate_process(args, type)
...
func _instantiate_process(p_args: Dictionary, p_type: StringName) -> DialogueSystemThreadProcessBase:
	var instance: DialogueSystemThreadProcessBase
	match p_type:
		&"Text":
...
func _instantiate_process_text(p_type: StringName, p_general_type: StringName) -> DialogueSystemThreadProcessBase:
	var process_name: String = "%s_%s" % [p_type, p_general_type]
	var scene: PackedScene = load(_a_PROCESS_SCENE_PATH % process_name)
	var instance: DialogueSystemThreadProcessBase = scene.instantiate()
...
func set_process_mode_(p_process_mode: ProcessMode) -> void:
	...
	for child: DialogueSystemThreadProcessBase in _a_Processes.get_children():
