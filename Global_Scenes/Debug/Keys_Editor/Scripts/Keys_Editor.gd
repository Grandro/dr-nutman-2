...
var _a_Key_Entry_Scene: PackedScene = preload("res://Global_Scenes/Debug/Keys_Editor/Key_Entry.tscn")

@onready var _a_HBox: HBoxContainer = get_node("HBox")
@onready var _a_Keys_Types: KeyEntryToggler = get_node("HBox/VBox/Keys_Types")
@onready var _a_Sort_By: SortBy = get_node("HBox/VBox/Options/Sort_By")
@onready var _a_Search: Search = get_node("HBox/VBox/Options/Search")
...
var _a_key_entry: DebugKeysEditorKeyEntry = null # Currently selected key_entry
var _a_keys_entries: Dictionary[StringName, DebugKeysEditorKeyEntry] = {} # Match key to key_entry
...
func _create_keys_types() -> void:
	for keys_type: StringName in [&"Map", &"Global"]:
		...
		var instance: EntryTogglerKeyEntry = _a_Keys_Types.instantiate_entry_(select_text, null, keys_type)
...
func _select_next_entry(p_idx: int) -> void:
	...
	if child_count > p_idx:
		var child: DebugKeysEditorKeyEntry = _a_Keys.get_child(p_idx)
		_select_entry(child)

func _select_entry(p_instance: DebugKeysEditorKeyEntry) -> void:
	p_instance.select()
...
func _update_key_entries() -> void:
	...
	for key: StringName in data:
		...
		var instance: DebugKeysEditorKeyEntry = _instantiate_key_entry(key, time_created, entry_data[&"Data"])
...
func _create_new_key_entry(p_key: StringName, p_data: Dictionary = {}) -> void:
	...
	var instance: DebugKeysEditorKeyEntry = _instantiate_key_entry(p_key, time_created, p_data)
	instance.ready.connect(_on_Key_Entry_ready)
	
	_a_Keys.add_child(instance)

func _instantiate_key_entry(p_key: StringName, p_time_created: float, p_data: Dictionary = {}) -> DebugKeysEditorKeyEntry:
	var instance: DebugKeysEditorKeyEntry = _a_Key_Entry_Scene.instantiate()
...
func _clear_key_entries() -> void:
	...
	for child: DebugKeysEditorKeyEntry in _a_Keys.get_children():
		child.queue_free()
...
func _on_Keys_Types_toggled(p_instance: EntryTogglerKeyEntry) -> void:
	_update_data()
...
func _on_Key_Entry_request_key(p_key: StringName, p_instance: DebugKeysEditorKeyEntry) -> void:
	if _is_key_used(p_key):
		var messages: Messages = Debug.get_messages()
		messages.show_info(tr(&"DEBUG_KEY_IN_USE"))
		return
...
func _on_Key_Entry_selected(p_instance: DebugKeysEditorKeyEntry) -> void:
...
func _on_Key_Entry_duplicate_pressed(p_instance: DebugKeysEditorKeyEntry) -> void:
...
func _on_Key_Entry_tree_exiting(p_instance: DebugKeysEditorKeyEntry) -> void:
