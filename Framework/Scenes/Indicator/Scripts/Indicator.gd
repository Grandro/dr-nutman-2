...
@onready var _a_Input: TextureRect = get_node("Margin/HBox/Input")
...
func _update_input_texture() -> void:
	var use_joy: bool = Global.get_use_joy()
	if use_joy:
		var joy_type: StringName = Global.get_joy_type()
		match joy_type:
...
func _on_Global_use_joy_changed() -> void:
	_update_input_texture()
