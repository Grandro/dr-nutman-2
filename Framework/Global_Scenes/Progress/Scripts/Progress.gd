...
@onready var _a_Pins: FWProgressPins = get_node("Pins")
@onready var _a_Quest_Info: FWProgressQuestInfo = get_node("Quest_Info")
@onready var _a_Loot_Info: FWProgressLootInfo = get_node("Loot_Info")

var _a_id: StringName = &"None" # ID used to identify story progress
var _a_chapter: String = &""
var _a_quests: Dictionary[StringName, FWProgressQuestBase] = {} # Match key to instance
var _a_objects: Dictionary[StringName, FWProgressObjectBase] = {} # Match key to instance
...
func start_quest(p_key: StringName) -> void:
	var instance: FWProgressQuestBase = _instantiate_quest(p_key)
...
func manual_progress_quest_objective(p_key: StringName, p_objective_name: String) -> void:
	var instance: FWProgressQuestBase = _a_quests[p_key]
	instance.manual_progress_objective(p_objective_name)
...
func call_object(p_key: StringName, p_method_name: StringName, p_args: Array = []) -> Variant:
	...
	var instance: FWProgressObjectBase = _a_objects[p_key]
	if !instance.has_method(p_method_name):
...
func _instantiate_quest(p_key: StringName) -> FWProgressQuestBase:
	var scene: PackedScene = load(_a_QUEST_SCENE_PATH % [p_key, p_key])
	var instance: FWProgressQuestBase = scene.instantiate()
...
func _instantiate_object(p_key: StringName) -> FWProgressObjectBase:
	var scene: PackedScene = load(_a_OBJECT_SCENE_PATH % [p_key, p_key])
	var instance: FWProgressObjectBase = scene.instantiate()
...
func _clear_quests() -> void:
	for child: FWProgressQuestBase in _a_Quests.get_children():
		_a_Quests.remove_child(child)
		child.queue_free()

func _clear_objects() -> void:
	for child: FWProgressObjectBase in _a_Objects.get_children():
...
func get_quests() -> Dictionary[StringName, FWProgressQuestBase]:
	return _a_quests
...
func set_id(p_id: StringName) -> void:
	_a_id = p_id
...
func is_quest_active(p_key: StringName) -> bool:
	...
	if started:
		var quest: FWProgressQuestBase = _a_quests[p_key]
...
func get_quest_objective_value(p_key: StringName, p_objective_name: String) -> Variant:
	var instance: FWProgressQuestBase = _a_quests[p_key]
	return instance.get_objective_value(p_objective_name)

func get_quest_objective_target_value(p_key: StringName, p_objective_name: String) -> Variant:
	var instance: FWProgressQuestBase = _a_quests[p_key]
	return instance.get_objective_target_value(p_objective_name)
...
func _get_quests_save_data() -> Dictionary:
	...
	for key: StringName in _a_quests:
		var instance: FWProgressQuestBase = _a_quests[key]
...
func _get_objects_save_data() -> Dictionary:
	...
	for key: StringName in _a_objects:
		var instance: FWProgressObjectBase = _a_objects[key]
		data[key] = instance.get_save_data()
...
func _load_quests_file_data(p_data: Dictionary) -> void:
	...
	for key: StringName in p_data:
		var instance: FWProgressQuestBase = _instantiate_quest(key)
		instance.load_file_data(p_data[key])

func _load_objects_file_data(p_data: Dictionary) -> void:
	_clear_objects()
	for key: StringName in p_data:
		var instance: FWProgressObjectBase = _instantiate_object(key)
		instance.load_file_data(p_data[key])
