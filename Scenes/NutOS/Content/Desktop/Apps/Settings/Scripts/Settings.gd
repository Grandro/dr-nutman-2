...
class_name NutOSContentDesktopAppSettings

signal app_installed(p_key: StringName)
signal app_uninstalled(p_key: StringName)

const _a_OPTIONS: Array[StringName] = [&"Apps", &"Keyboard", &"Background"]
const _a_OPTION_ICON_PATH: String = "res://Scenes/NutOS/Content/Desktop/Apps/Settings/Sprites/%s.png"
const _a_OPTION_HEADING: String = "NUTOS_APP_SETTINGS_OPTIONS_%s_HEADING"
...
@onready var _a_Menu: MarginContainer = get_node("Contents/Menu")

var _a_desktop: NutOSContentDesktop

var _a_menu_data: Dictionary = {} # Match menu key to save_data
var _a_menu_instance: NutOSContentDesktopAppSettingsMenuBase
...
func open(p_save_data: Dictionary) -> void:
	...
	for key: StringName in _a_menu_data:
		var args: Dictionary = _a_menu_data[key]
		if args.is_empty():
			continue
	...
	var pos: Vector2 = p_save_data[&"Pos"]
	set_position(pos)

func _instantiate_options() -> void:
	for key: StringName in _a_OPTIONS:
		var icon: Texture2D = load(_a_OPTION_ICON_PATH % key)
		var heading: String = _a_OPTION_HEADING % key.to_upper()
		var desc: String = _a_OPTION_DESC % key.to_upper()
		var instance: NutOSContentDesktopAppSettingsOptionEntry = _a_Option_Entry_Scene.instantiate()
...
func _instantiate_menu(p_key: StringName) -> void:
	var scene: PackedScene = load(_a_MENU_PATH % [p_key, p_key])
	...
	var heading: String = _a_OPTION_HEADING % p_key.to_upper()
	set_title(heading)
...
func get_save_data(p_closed: bool) -> Dictionary:
	var data: Dictionary = {}
	for child: NutOSContentDesktopAppSettingsMenuBase in _a_Menu.get_children():
...
func _on_Option_Entry_selected(p_key: StringName) -> void:
	_instantiate_menu(p_key)

func _on_Menu_return_pressed() -> void:
	var key: StringName = _a_menu_instance.get_key()
...
func _on_Menu_option_selected(p_key: StringName, p_option: StringName, p_value: Variant) -> void:
	option_selected.emit(p_key, p_option, p_value)

func _on_Apps_app_installed(p_key: StringName) -> void:
	app_installed.emit(p_key)

func _on_Apps_app_uninstalled(p_key: StringName) -> void:
