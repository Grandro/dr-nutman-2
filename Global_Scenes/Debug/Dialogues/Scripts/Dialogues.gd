extends DebugKeysEditorParts
class_name DebugDialogues
...
@onready var _a_Text: DebugDialoguesAttributeText = get_node("VBox/HBox/VBox/Attributes/Text")
@onready var _a_Info: DebugDialoguesAttributeInfo = get_node("VBox/HBox/VBox/Attributes/Info")
@onready var _a_Choice: DebugDialoguesAttributeChoice = get_node("VBox/HBox/VBox/Attributes/Choice")

var _a_attr_instance: DebugDialoguesAttributeBase = null
...
func _ready() -> void:
	...
	for child: DebugDialoguesAttributeBase in _a_Attributes.get_children():
		child.hide()
...
func _selected_part_changed(p_instance: DebugEntryListEntry) -> void:
	super(p_instance)
	_close_attributes()
	_open_attributes()
...
func _on_Parts_entry_type_changed(p_instance: DebugEntryListTypeEntry) -> void:
...
func MESSAGES_PROCEED(p_response: StringName, p_args: Array) -> void:
	if p_response == &"Yes":
		var type: StringName = p_args[0]
		if type == &"Part":
			var instance: DebugEntryListEntry = p_args[1]
			if _a_part == instance:
				_close_attributes()
				_a_part = null
			instance.queue_free()
