...
class_name DebugCommandEditPreviewDimensions3D

signal nav_map_ready()

var _a_preview_vp: VP
var _a_point: DebugValueSelectPoint3D
...
func _init(p_entity) -> void:
	...
	await _a_entity.get_tree().physics_frame
	await _a_entity.get_tree().physics_frame
	await _a_entity.get_tree().physics_frame
	await _a_entity.get_tree().physics_frame
	await _a_entity.get_tree().physics_frame
...
func open_load(p_data: Dictionary) -> void:
	var grid_step: Vector3 = p_data[&"Grid"][&"Step"]
	var grid_offset: Vector3 = p_data[&"Grid"][&"Offset"]
...
func update_point() -> void:
	...
	var point_scale: Vector3 = _a_grid_step.get_value()
...
func handle_free_camera_pan(p_relative: Vector2) -> void:
	var free_camera: Node3DObject = _a_entity.get_free_camera_instance()
...
func handle_free_camera_zoom_in(p_factor: float) -> void:
	var free_camera: Node3DObject = _a_entity.get_free_camera_instance()
	var camera_comp: CompCamera3D = free_camera.comph().get_comp("Camera")
...
func get_grid_length(p_grid_size: Vector2) -> int:
	var grid_step: Vector3 = _a_grid_step.get_value()
	var grid_length: int = int(ceil(p_grid_size.y / grid_step.z))
	
	return grid_length

func get_global_mouse_pos() -> Vector3:
	var mouse_pos: Vector2 = _a_preview_vp.get_mouse_position()
	var free_camera: Node3DObject = _a_entity.get_free_camera_instance()
	var camera_comp: CompCamera3D = free_camera.comph().get_comp("Camera")
	var collision_mask: int = 1 # Terrain
	var global_pos: Vector3 = Global.get_screen_pos_3D(mouse_pos, camera_comp, collision_mask)
	
	return global_pos

func _on_Grid_Step_z_value_changed(p_value: float) -> void:
