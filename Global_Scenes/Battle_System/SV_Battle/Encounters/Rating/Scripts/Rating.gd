...
const _a_RATING_LOC_ID: String = "SV_DAMAGE_RATING_%s"

@onready var _a_Text: RichTextLabel = get_node("VP/Text")
...
func display(p_pos: Vector3, p_rating: StringName) -> void:
	var rating_text: String = tr(_a_RATING_LOC_ID % p_rating.to_upper())
	var font: Font = _a_Text.get(&"theme_override_fonts/normal_font")
	var text_size: Vector2 = font.get_string_size(rating_text)
	...
	match p_rating:
		...
		&"Excellent":
			var prefix: String = "[rainbow freq=0.4 sat=0.8 val=0.8]"
