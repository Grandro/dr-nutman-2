...
class_name FWDebugActionEntryList
...
signal entry_selectable_focus_entered(p_instance: FWDebugEntryListActionEntry)

func instantiate_entry_(p_data: Array[Dictionary] = [], p_name: String = "") -> FWDebugEntryListActionEntry:
	var instance: FWDebugEntryListActionEntry = instantiate_entry(p_name)
...
func instantiate_entry_from_data(p_data: Dictionary) -> FWDebugEntryListActionEntry:
	var data: Array[Dictionary]; data.assign(p_data[&"Editor"])
	var name_: String = p_data[&"Name"]
	var instance: FWDebugEntryListActionEntry = instantiate_entry_(data, name_)
...
func _on_Entry_selectable_focus_entered(p_instance: FWDebugEntryListActionEntry) -> void:
	entry_selectable_focus_entered.emit(p_instance)

func _on_Add_pressed() -> void:
	var instance: FWDebugEntryListActionEntry = instantiate_entry_()
	add_entry(instance)
