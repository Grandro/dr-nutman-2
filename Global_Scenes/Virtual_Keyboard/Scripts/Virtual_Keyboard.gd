...
var _a_Keys_HBox_Scene: PackedScene = preload("res://Global_Scenes/Virtual_Keyboard/Keys_HBox.tscn")
var _a_Key_Scene: PackedScene = preload("res://Global_Scenes/Virtual_Keyboard/Key.tscn")
...
@onready var _a_Proceed_Text: Label = get_node("Control/Explanation/Proceed/Text")
...
func open(p_input: String = "") -> void:
	...
	var instance: VirtualKeyboardKey = hbox.get_child(0)
	instance.grab_focus()
	_a_input = p_input
...
func _update_key_instances(p_keyboard_layout: StringName) -> void:
	...
	var data: KeyboardLayoutData = Databases.get_data_entry(&"Keyboard_Layouts", p_keyboard_layout)
	for entry: Array in data.get_layout():
		...
		for idx: int in entry:
			...
			var instance: VirtualKeyboardKey = _a_Key_Scene.instantiate()
...
func _set_focus_neighbours() -> void:
	...
	for i: int in hboxs_count:
		...
		for j: int in key_count:
			var key_instance: VirtualKeyboardKey = hbox_instance.get_child(j)
			var down: VirtualKeyboardKey
			var left: VirtualKeyboardKey
			var right: VirtualKeyboardKey
			var up: VirtualKeyboardKey
...
func _set_keys_focus_mode(p_focus_mode: Control.FocusMode, p_except: VirtualKeyboardKey = null) -> void:
	for hbox_instance: HBoxContainer in _a_Keys_HBoxs.get_children():
		for key_instance: VirtualKeyboardKey in hbox_instance.get_children():
...
func _on_Hold_Down_timeout() -> void:
	...
	var next_instance: VirtualKeyboardKey = get_node(next_path)
...
func _on_Key_button_down(p_instance: VirtualKeyboardKey) -> void:
...
func _on_Key_focus_entered(p_instance: VirtualKeyboardKey) -> void:
	p_instance.set_self_modulate(_a_focused_color)
	_a_SFX_Dir_Pressed.play()

func _on_Key_focus_exited(p_instance: VirtualKeyboardKey) -> void:
