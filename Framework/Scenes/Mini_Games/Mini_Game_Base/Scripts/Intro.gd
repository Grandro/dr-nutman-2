...
class_name FWMiniGameBaseIntro

signal proceed_pressed()

@onready var _a_Heading: RichTextLabel = get_node("Margin/VBox/Heading")
@onready var _a_Proceed_Anims: AnimationPlayer = get_node("Margin/VBox/HBox/Right/Proceed/Anims")
@onready var _a_Anims: AnimationPlayer = get_node("Anims")

var _a_game_preview: FWMiniGameGameBase

var _a_proceed_pressed: bool = false
...
func _on_anim_finished(p_name: StringName) -> void:
	match p_name:
		&"Fade_In": set_process_unhandled_input(true)
		&"Fade_Out": _close()
