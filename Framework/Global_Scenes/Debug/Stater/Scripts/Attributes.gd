...
class_name FWDebugStaterAttributes

const _a_TYPES_LOC_ID: String = "FW_DEBUG_STATER_%s"

@onready var _a_Types: FWKeyEntryToggler = get_node("Types")
@onready var _a_Conditions: FWDebugExpressionEntryList = get_node("Conditions")
@onready var _a_Actions: FWDebugActionEntryList = get_node("Actions")
@onready var _a_Preview: FWDebugCommandsPreview = get_node("Preview")

var _a_key: StringName # Key_Entry key
var _a_action_entry: FWDebugEntryListActionEntry = null
...
func open(p_data: Dictionary) -> void:
	for type: StringName in p_data:
		var instance: FWDebugEntryList = _get_entry_list(type)
...
func _create_types() -> void:
	for type: String in ["Conditions", "Actions"]:
		...
		var instance: FWEntryTogglerKeyEntry = _a_Types.instantiate_entry_(select_text, null, type)
		_a_Types.add_entry(instance)

func _instantiate_condition(p_data: Dictionary = {}) -> void:
	var instance: FWDebugEntryListExpressionEntry = _a_Conditions.instantiate_entry_(p_data, _a_key)
	_a_Conditions.add_entry(instance)

func _instantiate_action(p_data: Array[Dictionary] = []) -> void:
	var instance: FWDebugEntryListActionEntry = _a_Actions.instantiate_entry_(p_data)
	_a_Actions.add_entry(instance)

func _get_entry_list(p_type: StringName) -> FWDebugEntryList:
	match p_type:
...
func get_save_data() -> Dictionary:
	...
	for type: StringName in [&"Conditions", &"Actions"]:
		var entry_list: FWDebugEntryList = _get_entry_list(type)
...
func _on_Types_toggled(p_instance: FWEntryTogglerKeyEntry) -> void:
...
func _on_Actions_entry_selectable_focus_entered(p_instance: FWDebugEntryListActionEntry) -> void:
