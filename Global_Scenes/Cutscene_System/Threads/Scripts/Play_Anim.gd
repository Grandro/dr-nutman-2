...
class_name CutsceneThreadPlayAnim
...
func skip() -> void:
	...
	if _a_wait_finish:
		var anims_comp: CompAnims = _a_object.comph().get_comp("Anims")
	...
	if _a_backwards:
		...
		# Jump to end of anim
		var curr_anim: String = _a_object.comph().call_comp("Anims", &"get_current_animation")
