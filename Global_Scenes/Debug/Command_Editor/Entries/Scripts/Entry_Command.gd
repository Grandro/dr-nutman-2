extends DebugCommandEditorEntryBase
class_name DebugCommandEditorEntryCommand
...
var _a_Arg_Entry_Scene: PackedScene = preload("res://Global_Scenes/Debug/Command_Editor/Entries/Arg_Entry.tscn")
...
func connect_to_editor(p_editor: DebugCommandEditor) -> void:
	super(p_editor)
...
func _update_display_main_base_desc() -> void:
	var base_desc_loc_id: StringName = "DEBUG_CUTSCENES_COMMANDS_%s" % _a_command.to_upper()
...
func _update_display_main_args() -> void:
	for child: DebugCommandEditorArgEntry in _a_Main.get_args_children():
		child.queue_free()

func _instantiate_main_arg(p_desc: String, p_color: Color) -> void:
	var margin: float = _get_main_arg_margin()
	var instance: DebugCommandEditorArgEntry = _a_Arg_Entry_Scene.instantiate()
...
func get_save_data() -> Dictionary:
	var data: Dictionary = get_editor_data()
...
func get_entries_count(p_branch_idx: int = -1) -> int:
	var entries: Array[DebugCommandEditorEntryBase] = get_entries(p_branch_idx)
	var count: int = entries.size()
	
	return count

func get_entries(p_branch_idx: int = -1) -> Array[DebugCommandEditorEntryBase]:
	var entries: Array[DebugCommandEditorEntryBase] = []
	if p_branch_idx == -1:
		for branch: DebugCommandEditorBranchBase in _a_Branches.get_children():
			for entry: DebugCommandEditorEntryBase in branch.get_entries():
				entries.push_back(entry)
	else:
		var branch: DebugCommandEditorBranchBase = _a_Branches.get_child(p_branch_idx)
		for entry: DebugCommandEditorEntryBase in branch.get_entries():
			entries.push_back(entry)
	
	return entries

func get_branch_entry(p_branch_idx: int, p_idx: int) -> DebugCommandEditorEntryBase:
	var branch: DebugCommandEditorBranchBase = _a_Branches.get_child(p_branch_idx)
	var entry: DebugCommandEditorEntryBase = branch.get_entry(p_idx)
