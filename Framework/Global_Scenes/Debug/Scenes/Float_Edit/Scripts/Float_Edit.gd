...
var _a_args: PackedStringArray = PackedStringArray(["0", "0"])
...
func _parse_value(p_value: Variant) -> PackedStringArray:
	match typeof(p_value):
		TYPE_FLOAT: p_value = str(p_value)
		TYPE_INT: p_value = str(p_value)
	
	var args: PackedStringArray = p_value.split(".")
	if args.size() == 1:
		args.push_back("0")
	
	return args

func _parsed_to_float(p_args: PackedStringArray) -> float:
	var left: float = float(p_args[0])
	var right: float = float(p_args[1])
...
func _update_text() -> void:
	var value: String = get_value_as_string()
	var pos: int = value.length()
	set_text(value)
...
func set_allow_lesser(p_allow: bool) -> void:
	if !p_allow:
		var value: float = _parsed_to_float(_a_args)
		if value < _e_min:
			set_value(_e_min)
...
func set_value(p_value: Variant, p_update: bool = true) -> void:
...
func get_value_as_string() -> String:
	var left: String = _a_args[0]
	var right: String = _a_args[1]
	var value: String = "%s.%s" % [left, right]
	
	return value

func _on_text_changed(p_text: String) -> void:
	if p_text.is_empty():
		set_value(0.0, false)
		return
	...
	var parsed: PackedStringArray = _parse_value(p_text)
	var value: float = _parsed_to_float(parsed)
	if !_e_allow_lesser:
		if value < _e_min:
			set_value(_e_min)
			return
