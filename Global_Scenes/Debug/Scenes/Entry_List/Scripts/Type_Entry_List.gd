...
class_name DebugTypeEntryList

signal entry_type_changing()
signal entry_type_changed(p_instance: DebugEntryListTypeEntry)

@export var _e_types: Array[StringName] = []

func instantiate_entry_(p_type: StringName = &"", p_data: Dictionary[StringName, Variant] = {}, p_name: String = "") -> DebugEntryListTypeEntry:
	var instance: DebugEntryListTypeEntry = instantiate_entry(p_name)
	if p_data.is_empty():
...
func instantiate_entry_from_data(p_data: Dictionary) -> DebugEntryListTypeEntry:
	...
	var instance: DebugEntryListTypeEntry = instantiate_entry_(type, data, name_)
...
func _on_Add_pressed() -> void:
	var instance: DebugEntryListTypeEntry = instantiate_entry_()
	add_entry(instance)

func _on_Entry_type_changing() -> void:
	entry_type_changing.emit()

func _on_Entry_type_changed(p_instance: DebugEntryListTypeEntry) -> void:
