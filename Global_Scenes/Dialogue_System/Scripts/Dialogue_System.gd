...
var _a_Thread_Scene: PackedScene = preload("res://Global_Scenes/Dialogue_System/Thread/Thread.tscn")
...
var _a_threads: Dictionary[StringName, DialogueSystemThread] = {} # Match dialogue key to thread instance
...
func dialogue(p_key: StringName, p_caller: Node = null, p_process_type: StringName = &"Main", p_fade_out: bool = true,
	...
	var instance: DialogueSystemThread = _instantiate_thread(p_key, p_caller, p_process_type, p_start_idx,
...
func manual_proceed(p_key: StringName) -> void:
	var thread: DialogueSystemThread = _a_threads[p_key]
...
func stop() -> void:
	_a_threads.clear()
	for child: DialogueSystemThread in _a_Threads.get_children():
		child.queue_free()
...
func _instantiate_thread(p_key: StringName, p_caller: Node, p_process_type: StringName, p_start_idx: int, p_end_idx: int,
						 p_fade_out: bool, p_key_type: StringName, p_instances_type: StringName, p_speech_bubbles = []) -> DialogueSystemThread:
	var instance: DialogueSystemThread = _a_Thread_Scene.instantiate()
...
func _update_threads_vox(p_instance: DialogueSystemThread, p_process_type: String) -> void:
...
func set_dialogue_process_mode(p_key: StringName, p_process_mode: ProcessMode) -> void:
	var instance: DialogueSystemThread = _a_threads[p_key]
...
func load_data(p_location: StringName) -> void:
	...
	for args: Dictionary in save_data[&"Threads"]:
		...
		var instance: DialogueSystemThread = _instantiate_thread(key, caller, process_type, start_idx,
...
func _on_Thread_completed(p_instance: DialogueSystemThread) -> void:
	...
	if process_type == &"Main":
		...
		var latest_main: DialogueSystemThread = null
		var latest_non_main: DialogueSystemThread = null
		var children: Array[Node] = _a_Threads.get_children()
		for i: int in range(children.size() - 1, -1, -1):
			var child: DialogueSystemThread = children[i]
