...
class_name DebugCommandEditor

signal option_test_selected(p_instance: DebugCommandEditorEntryBase)
signal selectable_focus_entered()

@export var _e_edit_scene_paths: Dictionary = {} # [Command][Dim] = path

var _a_Entry_Base_Scene: PackedScene = preload("res://Global_Scenes/Debug/Command_Editor/Entries/Entry_Base.tscn")

const _a_ENTRY_PATH: String = "res://Global_Scenes/Debug/Command_Editor/Entries/%s/%s.tscn"

@onready var _a_Entries: VBoxContainer = get_node("Contents/Panel/Entries")
@onready var _a_Options: DebugCommandEditorOptions = get_node("Contents/Panel/Options")
@onready var _a_Warnings: DebugCommandEditorWarnings = get_node("Contents/Panel/Warnings")

var _a_active: bool = false # Is currently active?
var _a_new_option: bool = false # Commands_List openend to create a new entry?
var _a_selected: Array[DebugCommandEditorEntryBase] = [] # Selected entries 
var _a_first: DebugCommandEditorEntryBase = null # First selected entry
var _a_last_focused: DebugCommandEditorEntryBase = null # Last focused entry
var _a_test_entry: DebugCommandEditorEntryBase = null # Entry with test mark
...
func update_entries(p_data: Array[Dictionary]) -> void:
	...
	var last: DebugCommandEditorEntryBase = _instantiate_empty_entry()
...
func clear_selected() -> void:
	for selected: DebugCommandEditorEntryBase in _a_selected:
		selected.set_fake_focus(false)
		selected.release_main_base_focus()
	_a_selected.clear()

func disable_last_entry() -> void:
	var last: DebugCommandEditorEntryBase = _a_Entries.get_child(-1)
	last.disable()

func _instantiate_empty_entry() -> DebugCommandEditorEntryBase:
	var instance: DebugCommandEditorEntryBase = _a_Entry_Base_Scene.instantiate()
	instance.connect_to_editor(self)
	
	return instance

func _instantiate_command_entry(p_command: StringName, p_data: Dictionary, p_args: Dictionary = {},
								p_margin: float = 4.0) -> DebugCommandEditorEntryBase:
	var path: String = _a_ENTRY_PATH % [p_command, p_command]
	var scene: PackedScene = load(path)
	var instance: DebugCommandEditorEntryBase = scene.instantiate()
	instance.connect_to_editor(self)
...
func _instantiate_command_entry_add(p_command: StringName, p_data: Dictionary, p_args: Dictionary) -> void:
	var instance: DebugCommandEditorEntryBase = _instantiate_command_entry(p_command, p_data, p_args)
...
func _option_new() -> void:
	_a_new_option = true
	var commands_list: DebugCommandsList = Debug.get_commands_list()
	commands_list.open()
...
func _option_edit() -> void:
	...
	var path: String = _e_edit_scene_paths[command][dim]
	var data: Dictionary = _a_first.get_data()
	var command_edit: DebugCommandEdit = Debug.get_command_edit()
	command_edit.open(_a_first, command, path, data, res_data)

func _option_cut() -> void:
	var to_copy: Array[DebugCommandEditorEntryBase] = _get_selected_copyable()
	if to_copy.is_empty():
		return
	
	var first_idx: int = _a_first.get_index()
	var last: DebugCommandEditorEntryBase = to_copy.back()
	var last_idx: int = last.get_index()
	var next_idx: int = max(first_idx, last_idx) + 1
	
	var clipboard: Array[Array] = []
	for instance: DebugCommandEditorEntryBase in to_copy:
		var command: StringName = instance.get_command()
		var data: Dictionary = instance.get_editor_data()
		var args: Array = [command, data]
	...
	if entry_count > next_idx:
		var next_focus: DebugCommandEditorEntryBase
		if parent == _a_Entries:
...
func _option_paste() -> void:
	var clipboard: Array[Array] = Debug.get_command_editor_clipboard()
	if clipboard.is_empty():
		return
	
	var parents: Array[DebugCommandEditorEntryBase] = _a_first.get_parents().duplicate()
	var branch_idx: int = _a_first.get_branch_idx()
	...
	for i: int in size_:
		...
		var args: Array = clipboard[idx]
		var command: StringName = args[0]
		var data: Dictionary = args[1]
		var command_data: Dictionary = data["Data"]
		var command_args: Dictionary = data["Args"]
		
		var instance: DebugCommandEditorEntryBase = _instantiate_command_entry(command, command_data, command_args, margin)
...
func _option_delete() -> void:
	var selected: Array[DebugCommandEditorEntryBase] = _get_selected_copyable()
	if selected.is_empty():
		return
	
	var first_idx: int = _a_first.get_index()
	var last: DebugCommandEditorEntryBase = selected.back()
	var last_idx: int = last.get_index()
	var next_idx: int = max(first_idx, last_idx) + 1
	
	for instance: DebugCommandEditorEntryBase in selected:
		instance.queue_free()
...
func _option_select_all() -> void:
	...
	for i: int in children.size() - 1:
		var child: DebugCommandEditorEntryBase = children[i]
		if i == 0:
...
func _shift_logic(p_clicked: DebugCommandEditorEntryBase) -> void:
	...
	var focused_parents: Array[DebugCommandEditorEntryBase] = _a_first.get_parents()
	var clicked_parents: Array[DebugCommandEditorEntryBase] = p_clicked.get_parents()
	var sel_entry: DebugCommandEditorEntryBase
	var nosel_entry: DebugCommandEditorEntryBase
	...
	for child: DebugCommandEditorEntryBase in children:
	...
	var sel_parents: Array[DebugCommandEditorEntryBase] = sel_entry.get_parents()
	var nosel_parents: Array[DebugCommandEditorEntryBase] = nosel_entry.get_parents()
	...
	for i: int in range(start_idx, end_idx + 1):
		var entry: DebugCommandEditorEntryBase
		if nearest_parent == _a_Entries:
	...
	if !_a_selected.has(_a_first):
		if focused_parents.size() > 0:
			var next_parent: DebugCommandEditorEntryBase = focused_parents[-1]
...
func _shift_arrows(p_shift: int) -> void:
	var next_focus: DebugCommandEditorEntryBase = _a_last_focused
	var parents: Array[DebugCommandEditorEntryBase] = _a_last_focused.get_parents()
	...
	for i: int in parents.size():
		...
		var parent: DebugCommandEditorEntryBase = parents[idx]
...
func is_entries_in_same_branch(p_focused: DebugCommandEditorEntryBase, p_clicked: DebugCommandEditorEntryBase) -> bool:
...
func _get_combined_entry_res_data(p_instance: DebugCommandEditorEntryBase) -> Dictionary:
	...
	var root: DebugCommandEditorEntryBase = p_instance
	var org_idx: int = p_instance.get_index()
	var end_idx: int = org_idx
	var parents: Array[DebugCommandEditorEntryBase] = p_instance.get_parents()
	if parents.size() > 0:
		# Nested entry
		root = parents[0]
	...
	for i: int in end_idx:
		var child: DebugCommandEditorEntryBase = _a_Entries.get_child(i)
		if child == root:
...
func _get_selected_real() -> Array[DebugCommandEditorEntryBase]:
	# First entry in a_selected could be nested too deep
	var entries: Array[DebugCommandEditorEntryBase] = []
	for entry: DebugCommandEditorEntryBase in _a_selected:
		if entry == _a_first:
			if _a_selected.size() > 1:
				var second: DebugCommandEditorEntryBase = _a_selected[1]
				var first_parents: Array[DebugCommandEditorEntryBase] = _a_first.get_parents()
				var second_parents: Array[DebugCommandEditorEntryBase] = second.get_parents()
...
func _get_selected_copyable() -> Array[DebugCommandEditorEntryBase]:
	# Command of entry could be empty
	var entries: Array[DebugCommandEditorEntryBase] = []
	var real_entries: Array[DebugCommandEditorEntryBase] = _get_selected_real()
	for entry: DebugCommandEditorEntryBase in real_entries:
		if !entry.is_empty():
			entries.push_back(entry)
	
	return entries

func _get_nearest_parent(p_entry: DebugCommandEditorEntryBase) -> Container:
	var nearest: Container = _a_Entries
	var parents: Array[DebugCommandEditorEntryBase] = p_entry.get_parents()
	if parents.size() > 0:
		nearest = parents[-1]
	
	return nearest

func _get_for_loop_idx_ords(p_instance: DebugCommandEditorEntryBase) -> Array[int]:
	var idx_ords: Array[int] = []
	var parents: Array[DebugCommandEditorEntryBase] = p_instance.get_parents()
	for parent: DebugCommandEditorEntryBase in parents:
...
func _get_sub_process_id_ords(p_instance: DebugCommandEditorEntryBase) -> Array[int]:
...
func get_skip_idxs(p_instance: DebugCommandEditorEntryBase = null) -> Array[int]:
	if p_instance == null:
...
func get_cutscene_data() -> Array[Dictionary]:
	...
	for child: DebugCommandEditorEntryBase in _a_Entries.get_children():
		...
		var data: Variant = child.get_cutscene_data()
...
func _on_Commands_List_command_selected(p_command: StringName) -> void:
	...
	var dim: StringName = Scene_Manager.get_curr_scene_dim()
	var path: String = _e_edit_scene_paths[p_command][dim]
	var command_edit: DebugCommandEdit = Debug.get_command_edit()
	if _a_new_option:
...
func _on_Command_Edit_command_ok(p_data: Dictionary, p_command: StringName) -> void:
	if _a_new_option:
		var margin: float = _a_first.get_branches_margin()
		var parent: Node = _a_first.get_parent()
		var instance: DebugCommandEditorEntryBase = _instantiate_command_entry(p_command, p_data, {}, margin)
		var parents: Array[DebugCommandEditorEntryBase] = _a_first.get_parents().duplicate()
		var branch_idx: int = _a_first.get_branch_idx()
		instance.set_parents(parents)
		instance.set_branch_idx(branch_idx)
...
func _on_Entry_selectable_focus_entered(p_instance: DebugCommandEditorEntryBase) -> void:
	if !Input.is_action_pressed(&"Shift"):
		for instance: DebugCommandEditorEntryBase in _a_selected:
...
func _on_Entry_selectable_left_clicked(p_instance: DebugCommandEditorEntryBase) -> void:
	if Input.is_action_pressed(&"Shift"):
		_shift_logic(p_instance)

func _on_Entry_selectable_right_clicked(p_pos: Vector2, p_instance: DebugCommandEditorEntryBase) -> void:
...
func _on_Entry_arg_focus_entered(p_instance: DebugCommandEditorEntryBase) -> void:
	if Input.is_action_pressed(&"Shift"):
		_shift_logic(p_instance)
	else:
		for instance: DebugCommandEditorEntryBase in _a_selected:
...
func _on_Entry_arg_right_clicked(p_pos: Vector2, p_instance: DebugCommandEditorEntryBase) -> void:
	if _a_selected.has(p_instance):
...
func _on_Entry_warning_pressed(p_pos: Vector2, p_instance: DebugCommandEditorEntryBase) -> void:
	_a_Warnings.open(p_pos, p_instance)

func _on_Entry_mark_changed(p_mark: StringName, p_instance: DebugCommandEditorEntryBase) -> void:
	match p_mark:
		&"Default":
...
func _on_Entry_request_empty_entry(p_branch_idx: int, p_margin: float, p_entries: VBoxContainer,
								   p_entry: DebugCommandEditorEntryBase) -> void:
	var instance: DebugCommandEditorEntryBase = _instantiate_empty_entry()
	var parents: Array[DebugCommandEditorEntryBase] = p_entry.get_parents().duplicate()
...
func _on_Entry_request_command_entry(p_command: StringName, p_data: Dictionary, p_branch_idx: int, p_margin: float,
									 p_entries: VBoxContainer, p_entry: DebugCommandEditorEntryBase) -> void:
	var instance: DebugCommandEditorEntryBase = _instantiate_command_entry(p_command, p_data[&"Data"], p_data[&"Args"], p_margin)
	var parents: Array[DebugCommandEditorEntryBase] = p_entry.get_parents().duplicate()
	parents.push_back(p_entry)
	instance.set_parents(parents)
	instance.set_branch_idx(p_branch_idx)
