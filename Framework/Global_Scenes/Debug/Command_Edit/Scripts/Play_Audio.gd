...
class_name FWDebugCommandEditCommandPlayAudio

@onready var _a_Audio_Type: FWDebugValueSelectOptions = get_node("Window/Contents/Margin/HBox/Right/Options/VBox/Audio_Type")
@onready var _a_Type: FWDebugValueSelectOptions = get_node("Window/Contents/Margin/HBox/Right/Options/VBox/Type")
@onready var _a_Audio: FWDebugValueSelectAudio = get_node("Window/Contents/Margin/HBox/Right/Options/VBox/Audio")
@onready var _a_Volume: FWDebugValueSelectFloat = get_node("Window/Contents/Margin/HBox/Right/Options/VBox/Volume")
@onready var _a_Pitch: FWDebugValueSelectFloat = get_node("Window/Contents/Margin/HBox/Right/Options/VBox/Pitch")
@onready var _a_Max_Distance: FWDebugValueSelectFloat = get_node("Window/Contents/Margin/HBox/Right/Options/VBox/Max_Distance")
@onready var _a_Wait_Finish: FWDebugValueSelectBool = get_node("Window/Contents/Margin/HBox/Right/Options/VBox/Wait_Finish")

var _a_type: StringName # Static/Object/Point
...
func open(p_instance, p_data: Dictionary, p_res_data: Dictionary) -> void:
...
func _update_audio_playback() -> void:
	if _a_Audio.is_audio_playing():
		var bus: StringName = _a_Audio_Type.get_selected_key()
		var stream: AudioStream = _a_Audio.get_audio_stream()
		var from_pos: float = _a_Audio.get_audio_playback_position()
		var volume: float = _a_Volume.get_value()
		var pitch: float = _a_Pitch.get_value()
		
		var free_audio: Node = _a_preview_scene.get_free_audio()
		match _a_type:
			&"Static":
				free_audio.stop()
			...
			&"Object":
				...
				var max_distance: float = _a_Max_Distance.get_value()
...
func _selected_audio_type_changed() -> void:
	var bus: StringName = _a_Audio_Type.get_selected_key()
	_a_Audio.set_bus(bus)
...
func _on_Audio_Type_selected() -> void:
	_selected_audio_type_changed()
...
func _on_Audio_paused() -> void:
	_update_audio_playback()

func _on_Audio_stopped() -> void:
	_update_audio_playback()

func _on_Audio_seeked(_p_pos: float) -> void:
	_update_audio_playback()

func _on_Volume_value_changed(_p_value: float) -> void:
	_update_audio_playback()

func _on_Pitch_value_changed(_p_value: float) -> void:
	_update_audio_playback()

func _on_Max_Distance_value_changed(_p_value: float) -> void:
	_update_audio_playback()

func _on_Attenuation_value_changed(_p_value: float) -> void:
	_update_audio_playback()
