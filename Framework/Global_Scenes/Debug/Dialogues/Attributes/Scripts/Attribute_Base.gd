...
class_name FWDebugDialoguesAttributeBase

func open(p_data: Dictionary) -> void:
	for child: FWDebugDialoguesAttributesTabBase in get_children():
		if p_data.is_empty():
			child.open_init()
		else:
...
func _set_tab_hidden(p_instance: FWDebugDialoguesAttributesTabBase, p_hidden: bool) -> void:
