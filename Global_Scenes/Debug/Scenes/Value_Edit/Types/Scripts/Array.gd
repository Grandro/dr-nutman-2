...
class_name DebugValueEditTypeArray
...
func _update_value_entries() -> void:
	...
	if diff > 0:
		for i: int in range(entries_count, value_size):
			var instance: DebugValueEdit = _instantiate_value_edit(i)
		...
		for i: int in range(entries_count - 1, value_size - 1, -1):
			var child: DebugValueEdit = _a_Entries.get_child(i)
...
func _delete_value_entry(p_instance: DebugValueEdit) -> void:
...
func _instantiate_value_edit(p_idx: int) -> DebugValueEdit:
	var instance: DebugValueEdit = _a_Value_Edit_Scene.instantiate()
	var child_data: ValueEditData = _a_data.get_arr_child_data_idx(p_idx)
...
func _set_data(p_data: ValueEditData) -> void:
	var child_data: Array[ValueEditData] = p_data.get_arr_child_data()
...
func _on_Value_Edit_value_changed(p_value: Variant, p_instance: DebugValueEdit) -> void:
...
func _on_Value_Edit_tree_exiting(p_instance: DebugValueEdit) -> void:
	var idx: int = p_instance.get_index()
