...
class_name DebugCommandEditCommandMatch
...
@onready var _a_Branches: DebugValueEditEntryList = get_node("Window/Contents/Margin/VBox/VBox/Branches/Parts")
...
func _update_branches(p_editable: bool, p_values: Array) -> void:
	...
	for i: int in size_:
		var instance: DebugEntryListEntry
		if entries.size() <= i:
...
func _branches_values_changed(p_instance: DebugCommandEditMenuBase) -> void:
...
func _update_menu_data() -> void:
	var menu_instance: DebugCommandEditMenuBase = _a_menus[_a_key]
	var branches_values: Array = []
	for instance: DebugEntryListEntry in _a_Branches.get_entries():
