...
class_name NutOSContentDesktopAppSettingsMenuAppsAppEntry

signal installed()
signal uninstalled()

const _a_INSTALLED_APP_TEXTURE_PATH: String = "res://Scenes/NutOS/Content/Desktop/Apps/Sprites/Installed/%s.png"
const _a_NAME_LOC_ID: String = "NUTOS_APP_%s"
...
@onready var _a_Date: Label = get_node("HBox/VBox/Date")
@onready var _a_Install: Button = get_node("HBox/Install")

var _a_key: StringName = &""
var _a_installed: bool = false
var _a_unix_time: float = -1.0
...
func handle_app_opened() -> void:
	_a_Install.set_disabled(true)

func handle_app_closed() -> void:
	_a_Install.set_disabled(false)

func _update_icon(p_installed: bool) -> void:
...
func _update_date(p_installed: bool) -> void:
	if p_installed:
		var datetime = Time.get_datetime_dict_from_unix_time(int(_a_unix_time))
		var year: int = datetime[&"year"]
		var month: int = datetime[&"month"]
		var day: int = datetime[&"day"]
		var date_text: String = Global.get_date_text(year, month, day)
		_a_Date.set_text(date_text)
	else:
		_a_Date.set_text("-")

func _update_install(p_installed: bool) -> void:
	if p_installed:
...
func _set_installed(p_installed: bool) -> void:
