...
var _a_Default_Cursor: Texture2D = preload("uid://4ciygn1ar5uv")
var _a_Trans_Shader: ShaderMaterial = preload("uid://p1br5wo0d1l3")

const _a_SAVES_PATH: String = "user://Saves/%s"
const _a_SAVE_PATH: String = "user://Saves/%s/%s.save"
const _a_TITLE_SCREEN_SCENE_UID: String = "uid://dv7xgla30k3pa"
...
func _update_party_member_stats(p_key: StringName, p_stats: FWStatsData, p_equip: bool) -> void:
	...
	if p_equip:
		set_party_member_stat(p_key, &"Max_HP", max_HP + p_stats.get_stat(&"HP"))
		set_party_member_stat(p_key, &"Max_SP", max_SP + p_stats.get_stat(&"SP"))
		set_party_member_stat(p_key, &"ATK", ATK + p_stats.get_stat(&"ATK"))
		set_party_member_stat(p_key, &"MAG", MAG + p_stats.get_stat(&"MAG"))
		set_party_member_stat(p_key, &"DEF", DEF + p_stats.get_stat(&"DEF"))
		set_party_member_stat(p_key, &"SPEED", SPEED + p_stats.get_stat(&"SPEED"))
		set_party_member_stat(p_key, &"LUCK", LUCK + p_stats.get_stat(&"LUCK"))
	else:
		set_party_member_stat(p_key, &"Max_HP", max_HP - p_stats.get_stat(&"HP"))
		set_party_member_stat(p_key, &"Max_SP", max_SP - p_stats.get_stat(&"SP"))
		set_party_member_stat(p_key, &"ATK", ATK - p_stats.get_stat(&"ATK"))
...
func equip_party_member(p_pm_key: StringName, p_group: StringName, p_key: StringName) -> void:
	...
	if !p_key.is_empty():
		var data: FWEquipableData = Databases.get_data_entry(&"Items", p_key)
		var stats: FWStatsData = data.get_stats()
...
func unequip_party_member(p_pm_key: StringName, p_group: StringName) -> void:
	...
	if key != &"":
		var data: FWEquipableData = Databases.get_data_entry(&"Items", key)
		var stats: FWStatsData = data.get_stats()
...
func change_item_amount(p_key: StringName, p_amount: int) -> void:
	var data: FWItemData = Databases.get_data_entry(&"Items", p_key)
...
func roll_loot(p_loot: Dictionary) -> Dictionary[StringName, int]:
	...
	var loot_RNG: FWDicRNG = FWDicRNG.new()
...
func _create_save_dirs() -> void:
	...
	for i: int in range(1, 4):
		_create_save_dir(str(i), dir)

func _create_save_dir(p_key: String, p_dir: DirAccess) -> void:
	var path: String = _a_SAVES_PATH % p_key
	if !FileAccess.file_exists(path):
		p_dir.make_dir_recursive(path)
...
func _init_party_members() -> void:
	...
	for key: StringName in pm_data:
		...
		# Stats
		var stats: FWStatsData = args.get_stats()
		_a_party_members[key][&"Stats"] = {}
		var stats_args: Dictionary = _a_party_members[key][&"Stats"]
		stats_args[&"Max_HP"] = stats.get_stat(&"Max_HP")
		stats_args[&"HP"] = stats.get_stat(&"HP")
		stats_args[&"Max_SP"] = stats.get_stat(&"Max_SP")
		stats_args[&"SP"] = stats.get_stat(&"SP")
		stats_args[&"ATK"] = stats.get_stat(&"ATK")
		stats_args[&"MAG"] = stats.get_stat(&"MAG")
		stats_args[&"DEF"] = stats.get_stat(&"DEF")
		stats_args[&"SPEED"] = stats.get_stat(&"SPEED")
		stats_args[&"LUCK"] = stats.get_stat(&"LUCK")
...
func get_screen_pos_3D(p_screen_pos: Vector2, p_camera: Camera3D, p_collision_mask: int) -> Variant:
...
func get_title_screen_scene_uid() -> String:
	return _a_TITLE_SCREEN_SCENE_UID
...
func save_data_object(p_instance: Node) -> void:
	var scene_manager_si: Scene_Manager = get_singleton(self, "Scene_Manager")
	var location: StringName = scene_manager_si.get_location()
	var maps_data: Dictionary = _a_saved_data[&"Maps"]
	if !maps_data.has(location):
		maps_data[location] = {}
		maps_data[location][&"Objects"] = {}
	p_instance.comph().call_comp("Save", &"save_data", [maps_data[location]])

func save_file_data_idx(p_save_file_idx: int) -> void:
	var path: String = _a_SAVES_PATH % p_save_file_idx
	var dir: DirAccess = DirAccess.open(path)
	if dir.file_exists("Auto.save"):
		dir.remove("Auto.save")
	
	_save_file_data(p_save_file_idx, "Save")

func save_file_data_auto() -> void:
	var save_file_idx: int = Global_Data.get_save_file_idx()
	_save_file_data(save_file_idx, "Auto")

func _save_file_data(p_save_file_idx: int, p_key: String) -> void:
	_a_saved_data[&"Singletons"][&"Global"] = _get_save_file_data()
	_a_saved_data[&"Version"] = get_version()
	save_data(true)
	
	var path: String = _a_SAVE_PATH % [p_save_file_idx, p_key]
	Data_Parser.write_var_data(path, _a_saved_data)

func load_file_data(p_save_file_idx: int, p_key: String) -> void:
	var path: String = _a_SAVE_PATH % [p_save_file_idx, p_key]
...
func sort_high_nested(p_a: Array, p_b: Array) -> bool:
	return p_a[0] > p_b[0]
