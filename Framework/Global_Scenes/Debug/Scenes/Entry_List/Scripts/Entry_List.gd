...
class_name FWDebugEntryList

signal entry_moved(p_old_idx: int, p_new_idx: int)
signal entry_select_pressed(p_instance: FWDebugEntryListEntry)
signal entry_tree_entered(p_instance: FWDebugEntryListEntry)
signal entry_tree_exited(p_idx: int)
signal entry_deleting(p_instance: FWDebugEntryListEntry)

@export var _e_entry_scene: PackedScene = preload("uid://ek1x7if6akb8")
...
@onready var _a_Search: FWSearch = get_node("Search")
@onready var _a_Scroll: ScrollContainer = get_node("Scroll")
@onready var _a_VBox: FWAnimVBox = get_node("Scroll/VBox")
@onready var _a_Add: Button = get_node("Add")

var _a_entries: Dictionary[String, FWDebugEntryListEntry] = {} # Match name to instance
...
func clear_entries() -> void:
	_a_entries.clear()
	for child: FWDebugEntryListEntry in _a_VBox.get_children():
		child.tree_exiting.disconnect(_on_Entry_tree_exiting)
		_a_VBox.remove_child(child)
		child.queue_free()

func instantiate_entry(p_name: String = "") -> FWDebugEntryListEntry:
	var can_change_name: bool = _e_can_change_name && !_e_enumerate
	var instance: FWDebugEntryListEntry = _e_entry_scene.instantiate()
...
func instantiate_entry_from_data(p_data: Dictionary) -> FWDebugEntryListEntry:
	var name_: String = p_data[&"Name"]
	var instance: FWDebugEntryListEntry = instantiate_entry(name_)
	
	return instance

func add_entry(p_instance: FWDebugEntryListEntry) -> void:
...
func _update_enumeration() -> void:
	...
	for i: int in _a_VBox.get_child_count():
		var child: FWDebugEntryListEntry = _a_VBox.get_child(i)
...
func _drop_data(p_position: Vector2, p_instance: Variant) -> void:
	...
	for i: int in children.size() - 1:
		var first: FWDebugEntryListEntry = children[i]
		var second: FWDebugEntryListEntry = children[i + 1]
...
func get_entry(p_idx: int) -> FWDebugEntryListEntry:
...
func load_data(p_data: Dictionary) -> void:
	...
	for entry_data: Dictionary in p_data.values():
		var instance: FWDebugEntryListEntry = instantiate_entry_from_data(entry_data)
...
func _on_Entry_up_pressed(p_instance: FWDebugEntryListEntry) -> void:
	var old_idx: int = p_instance.get_index()
...
func _on_Entry_down_pressed(p_instance: FWDebugEntryListEntry) -> void:
	var child_count: int = _a_VBox.get_child_count()
	var old_idx: int = p_instance.get_index()
	var new_idx: int = old_idx + 1
...
func _on_Entry_select_pressed(p_instance: FWDebugEntryListEntry) -> void:
	entry_select_pressed.emit(p_instance)

func _on_Entry_duplicate_pressed(p_instance: FWDebugEntryListEntry) -> void:
	var data: Dictionary = p_instance.get_save_data()
	var instance: FWDebugEntryListEntry = instantiate_entry_from_data(data.duplicate(true))
	add_entry(instance)

func _on_Entry_delete_pressed(p_instance: FWDebugEntryListEntry) -> void:
	if _e_confirm_delete:
		var messages: Messages = Debug.get_messages()
		messages.show_proceed(tr(_e_delete_loc_id), _CB_Messages_Proceed.bind(p_instance))
	else:
		entry_deleting.emit(p_instance)
		_a_VBox.remove_child(p_instance)
		p_instance.queue_free()

func _on_Entry_name_requested(p_name: String, p_instance: FWDebugEntryListEntry) -> void:
	if _e_ensure_unique_names:
...
func _on_Entry_tree_entered(p_instance: FWDebugEntryListEntry) -> void:
	entry_tree_entered.emit(p_instance)

func _on_Entry_tree_exiting(p_instance: FWDebugEntryListEntry) -> void:
...
func _CB_Messages_Proceed(p_response: StringName, p_instance: FWDebugEntryListEntry) -> void:
	if p_response == &"Yes":
		entry_deleting.emit(p_instance)
		_a_VBox.remove_child(p_instance)
		p_instance.queue_free()
