extends Node3D
class_name FWCompSpeechBubble3D

signal choice_selected(p_value: Variant)

@export var _e_margin: Vector2 = Vector2(8, 8)

@onready var _a_Panel: FWGUI3DPanel = get_node("Panel")
@onready var _a_Plane: MeshInstance3D = get_node("Panel/Plane")
@onready var _a_VP: FWVP = get_node("Panel/VP")
@onready var _a_Speech_Bubble_UI: FWCompSpeechBubbleUI = get_node("Panel/VP/Speech_Bubble_UI")

var _a_pos_px: _Positions = _Positions.new()
var _a_pos_m_old: _Positions = _Positions.new()
var _a_pos_vp: _Positions = _Positions.new()
var _a_pos_m_new: _Positions = _Positions.new()
var _a_panel_offset: Vector3 = Vector3.ZERO
var _a_arrow_flip_v: bool = false
...
func _process(_p_delta: float) -> void:
	...
	var curr_camera: Camera3D = global_si.get_curr_camera()
	var vp_size: Vector2i = global_si.get_base_vp_size()
	var panel_vp_size: Vector2i = _a_VP.get_size_2d_override()
	var plane_mesh: PlaneMesh = _a_Plane.get_mesh()
	var plane_mesh_size: Vector2 = plane_mesh.get_size()
	var px_per_m: Vector2 = Vector2(panel_vp_size) / plane_mesh_size
...
func init(p_entities: Array[Node]) -> void:
	...
	if entity_comph.has_comp("Interactions"):
		var interactions_comp: FWCompInteractions3D = entity_comph.get_comp("Interactions")
...
func _2D_to_3D_text_box(p_plane_mesh_size: Vector2, p_px_per_m: Vector2) -> void:
	# Text_Box
...
func _3D_to_vp_text_box(p_curr_camera: Camera3D) -> void:
	for i: int in 4:
		_a_pos_vp.text_box[i] = p_curr_camera.unproject_position(_a_pos_m_old.text_box[i])

func _vp_adjust_text_box(p_vp_size: Vector2i) -> void:
	# Left
...
func _vp_to_3D_text_box(p_curr_camera: Camera3D) -> bool:
	var normal: Vector3 = global_transform.basis.z.normalized()
	var origin: Vector3 = global_transform.origin
	var plane: Plane = Plane(normal, origin)
	
	_a_panel_offset = Vector3.ZERO
	for i: int in 4:
		var ray_from: Vector3 = p_curr_camera.project_ray_origin(_a_pos_vp.text_box[i])
		var ray_to: Vector3 = ray_from + p_curr_camera.project_ray_normal(_a_pos_vp.text_box[i])
...
func _2D_to_3D_arrow(p_plane_mesh_size: Vector2, p_px_per_m: Vector2) -> void:
	# Arrow
...
func _3D_to_vp(p_curr_camera: Camera3D) -> void:
	for i: int in 4:
		_a_pos_vp.text_box[i] = p_curr_camera.unproject_position(_a_pos_m_old.text_box[i])
		_a_pos_vp.arrow[i] = p_curr_camera.unproject_position(_a_pos_m_old.arrow[i])

func _vp_adjust_arrow(p_vp_size: Vector2i) -> void:
	# Left
...
func _vp_to_3D_arrow(p_curr_camera: Camera3D) -> bool:
	var normal: Vector3 = global_transform.basis.z.normalized()
	var origin: Vector3 = global_transform.origin
	var plane: Plane = Plane(normal, origin)
	...
	for i: int in 4:
		var ray_from: Vector3 = p_curr_camera.project_ray_origin(_a_pos_vp.arrow[i])
		var ray_to: Vector3 = ray_from + p_curr_camera.project_ray_normal(_a_pos_vp.arrow[i])
...
func _3D_to_2D(p_plane_mesh_size: Vector2, p_px_per_m: Vector2) -> void:
	var arrow_pos_tl: Vector2 = _m_to_pixel(_a_pos_m_new.arrow[CORNER_TOP_LEFT], p_plane_mesh_size, p_px_per_m)
	_a_Speech_Bubble_UI.set_arrow_global_pos(arrow_pos_tl)

func _vp_adjust_left_pos(p_pos: Vector2, p_border: float, p_margin: Vector2) -> Vector2:
	if p_pos.x < p_border + p_margin.x:
		var diff: float = p_border + p_margin.x - p_pos.x
		p_pos.x += diff
	
	return p_pos

func _vp_adjust_right_pos(p_pos: Vector2, p_border: float, p_margin: Vector2) -> Vector2:
	if p_pos.x > p_border - p_margin.x:
		var diff: float = p_pos.x - p_border + p_margin.x
		p_pos.x -= diff
	
	return p_pos

func _vp_adjust_top_pos(p_pos: Vector2, p_border: float, p_margin: Vector2) -> Vector2:
	if p_pos.y < p_border + p_margin.y:
		var diff: float = p_border + p_margin.y - p_pos.y
		p_pos.y += diff
	
	return p_pos

func _vp_adjust_bottom_pos(p_pos: Vector2, p_border: float, p_margin: Vector2) -> Vector2:
	if p_pos.y > p_border - p_margin.y:
		var diff: float = p_pos.y - p_border + p_margin.y
		p_pos.y -= diff
	
	return p_pos

func _px_to_m(p_pos_px: Vector2, p_plane_mesh_size: Vector2, p_px_per_m: Vector2) -> Vector3:
	var pos_m: Vector3 = Vector3.ZERO
...
func _m_to_pixel(p_pos_m: Vector3, p_plane_mesh_size: Vector2, p_px_per_m: Vector2) -> Vector2:
...
func _on_Interactions_interaction_activated(p_area: FWCompInteractionsInteraction3D) -> void:
	var speech_bubble_pos: Vector3 = p_area.get_speech_bubble_pos()
