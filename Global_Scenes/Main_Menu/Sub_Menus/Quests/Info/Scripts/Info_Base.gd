...
func display(p_key: StringName) -> void:
	...
	var quest_args: QuestData = Databases.get_data_entry(&"Quests", p_key)
	...
	var quest_progress: ProgressQuestBase = quests_progress[p_key]
	var quest_progress_obj: Array[Node] = quest_progress.get_objective_instances()
	for instance: ProgressQuestObjectiveBase in quest_progress_obj:
...
func _instantiate_objective(p_objective_instance: ProgressQuestObjectiveBase) -> void:
	var data: ObjectiveData = p_objective_instance.get_data()
