extends FWDebugValueSelectOptions
class_name FWDebugCommandEditCommandShowPopupType

func update_options() -> void:
	_clear_options()
	
	var popup_types: Array[StringName] = Global.get_popup_types()
	for i: int in popup_types.size():
		var type: StringName = popup_types[i]
		var loc_id: StringName = _e_options_loc_id % type.to_upper()
		_a_option_idxs[type] = i
		_a_Value.add_item(tr(loc_id))
		_a_Value.set_item_metadata(i, type)
