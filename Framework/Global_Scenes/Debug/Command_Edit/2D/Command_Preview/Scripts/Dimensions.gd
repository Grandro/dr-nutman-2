...
class_name FWDebugCommandEditCommandPreviewDimensions2D
...
func handle_free_camera_pan(p_relative: Vector2) -> void:
	var free_camera: FWNode2DObject = _a_entity.get_free_camera_instance()
	free_camera.position -= p_relative

func handle_free_camera_zoom_in(p_factor: float) -> void:
	var free_camera: FWNode2DObject = _a_entity.get_free_camera_instance()
	var camera_comp: FWCompCamera2D = free_camera.comph().get_comp("Camera")
