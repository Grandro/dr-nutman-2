...
class_name DebugCommandEditorEntryBranch
...
@onready var _a_End: DebugCommandEditorArgEntry = get_node("HBox/VBox/End")
...
func add_res_data(p_res_data: Dictionary, p_args: Array = []) -> void:
	...
	var entries: Array[DebugCommandEditorEntryBase] = get_entries(branch_idx)
	for i: int in entries.size():
		if i == end_idx:
			break
		
		var entry: DebugCommandEditorEntryBase = entries[i]
...
func _init_branches_data(p_data: Dictionary, p_margin: float) -> void:
	var child_count: int = _a_Branches.get_child_count()
	for i: int in range(_e_start_branch_idx, child_count):
		var branch: DebugCommandEditorBranchBase = _a_Branches.get_child(i)
...
func get_cutscene_data():
	var branch: DebugCommandEditorBranchBase = _a_Branches.get_child(_a_process_branch_idx)
	var data: Array[Dictionary] = branch.get_cutscene_data()
