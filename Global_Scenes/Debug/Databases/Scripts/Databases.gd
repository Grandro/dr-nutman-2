...
class_name DebugDatabases

const _a_KEY_LOC_ID: String = "DEBUG_DATABASES_%s"

@onready var _a_Keys: VBoxContainer = get_node("HBox/Keys")
@onready var _a_Value: DebugValueEdit = get_node("HBox/Scroll/Value")

func _ready() -> void:
	for child: DebugDatabasesKeyEntry in _a_Keys.get_children():
