...
class_name FWDebugLocEditorGroupEntry

signal entry_select_focus_entered(p_instance: FWDebugLocEditorLocIDEntry)
signal entry_loc_id_changed(p_loc_id: StringName, p_instance: FWDebugLocEditorLocIDEntry)
...
func instantiate_entry_(p_loc_id: StringName) -> FWDebugLocEditorLocIDEntry:
	var instance: FWDebugLocEditorLocIDEntry = instantiate_entry(p_loc_id)
...
func update_entries() -> void:
	for loc_id: StringName in _a_entries:
		var instance: FWDebugLocEditorLocIDEntry = _a_entries[loc_id]
...
func grab_first_entry_focus() -> void:
	for child: FWDebugLocEditorLocIDEntry in _a_VBox.get_children():
		if child.is_visible():
			child.grab_select_focus()
			break

func grab_entry_focus(p_loc_id: StringName) -> void:
	var instance: FWDebugLocEditorLocIDEntry = _a_entries[p_loc_id]
	instance.grab_select_focus()

func add_loc_id(p_loc_id: StringName) -> void:
	_a_loc_ids.push_back(p_loc_id)
	if _a_expanded:
		var instance: FWDebugLocEditorLocIDEntry = instantiate_entry_(p_loc_id)
		add_entry(instance)

func remove_loc_id(p_loc_id: StringName) -> void:
	_a_loc_ids.erase(p_loc_id)
	if _a_expanded:
		var instance: FWDebugEntryListGroupEntry = _a_entries[p_loc_id]
		instance.queue_free()

func _instantiate_entries() -> void:
	for loc_id: StringName in _a_loc_ids:
		var instance: FWDebugLocEditorLocIDEntry = instantiate_entry_(loc_id)
		add_entry(instance)
...
func get_first_entry() -> FWDebugLocEditorLocIDEntry:
	for child: FWDebugLocEditorLocIDEntry in _a_VBox.get_children():
		if child.is_visible():
			return child
...
func get_entry_(p_loc_id: StringName) -> FWDebugLocEditorLocIDEntry:
	return _a_entries[p_loc_id]

func get_entries_() -> Dictionary[String, FWDebugEntryListEntry]:
	return _a_entries

func _on_Expand_pressed() -> void:
	set_expanded(!_a_expanded)

func _on_Entry_select_focus_entered(p_instance: FWDebugLocEditorLocIDEntry) -> void:
	entry_select_focus_entered.emit(p_instance)

func _on_Entry_loc_id_changed(p_loc_id: String, p_instance: FWDebugLocEditorLocIDEntry) -> void:
